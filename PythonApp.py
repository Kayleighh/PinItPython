...
# Keeps trying to connect when connection failed.
...
def on_message(client, userdata, msg):
    ...
    if "/PinIt/Inf/PresenceRequest/" in msg.topic:
        ...
        for row in cur:
            ...
            # Publish all users that are available.
            client.publish(RTopic, payload= RPayload, qos=0, retain=False)
    
    # Code for users to check the presence state of teachers.
    ...
    # ID generator to create an unique ID in the database.
    def createID():
    ...
    # Receives an account request which is created in the database.
    # Republish it's unique ID.
    ...
    # Activates account when the users Android ID is not existing.
    # Then it updates the user account with it's unique Android ID.
    ...
    # Checks if account is valid with it's Android and User ID.
